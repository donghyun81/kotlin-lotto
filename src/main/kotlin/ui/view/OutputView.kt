...
class OutputView {
    ...
    fun printWinningResults(ranks: Map<LottoRank, Int>) {
        println(PRINT_WINNING_RESULT_HEADER)
        LottoRank.entries.sortedBy { it.prize }.drop(DROP_MISS_RANK_INDEX).forEach { lottoRank ->
    ...
    fun printWinningNumberMissing() {
        println("당첨 번호를 입력하지 않았습니다.")
    }

    fun printInvalidMessage() {
        println("유효하지 않은 입력 값입니다.")
    }
