...
class StatisticTest {
    ...
    @Test
    fun `당첨 순위를 구하는 기능 테스트`() {
        assertAll(
            { assertEquals(statistic.getRank(Lotto(listOf(1, 2, 3, 4, 5, 6))), Pair(6, false)) },
            { assertEquals(statistic.getRank(Lotto(listOf(1, 2, 3, 4, 5, 7))), Pair(5, true)) },
            { assertEquals(statistic.getRank(Lotto(listOf(1, 2, 3, 4, 5, 8))), Pair(5, false)) },
        )
    }
